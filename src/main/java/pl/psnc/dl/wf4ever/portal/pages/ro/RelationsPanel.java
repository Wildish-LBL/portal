...
public class RelationsPanel extends Panel {
	...
	public RelationsPanel(String id, IModel<ResearchObject> model) {
		...
		add(form);
		form.add(new AnnotationEditAjaxEventButton("annotate", form, model, this,
				AddAnnotationClickedEvent.class));
		form.add(new AnnotationTripleList("annotation-triple",
				new PropertyModel<List<AnnotationTriple>>(annotationCollection, "list")));
	...
	private void onAnnotateClicked(AddAnnotationClickedEvent event) {
		NewRelationTextPanel newRelationPanel = new NewRelationTextPanel("new-annotation", roModel,
				true, subjectsList, RelationsDict.predefinedRelations, objectsList);
	...
	final class AnnotationTripleList extends ListView<AnnotationTriple> {
		 ...
		 */
		public AnnotationTripleList(String id,
				IModel<? extends List<? extends AnnotationTriple>> model) {
			super(id, model);
		}
		...
		protected void populateItem(ListItem<AnnotationTriple> item) {
			...
			item.add(new EditableRelationTextPanel("editable-annotation-triple",
					new AnnotationTripleModel(item.getModelObject()), false));
	...
	final static class RelationsDict implements Serializable {
		...
		public static final List<URI> predefinedRelations = Arrays.asList(new URI[] {
				URI.create("Choose-Relation"), URI.create(PROV.wasDerivedFrom.getURI()),
				URI.create(PROV.wasRevisionOf.getURI()), URI.create(PROV.wasQuotedFrom.getURI()),
				URI.create(PROV.hadOriginalSource.getURI()),
				URI.create(ROTERMS.inputSelected.getURI()),
				URI.create(ROTERMS.subsequentWorkflow.getURI()),
				URI.create(ROTERMS.previousWorkflow.getURI()),
				...
				URI.create(WFPROV.describedByWorkflow.getURI()) });
