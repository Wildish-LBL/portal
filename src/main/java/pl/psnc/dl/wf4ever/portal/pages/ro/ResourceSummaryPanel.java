...
public class ResourceSummaryPanel extends Panel {
	 ...
	 */
	public ResourceSummaryPanel(String id, IModel<Resource> model,
			IModel<Folder> currentFolderModel, IModel<ResearchObject> roModel) {
		super(id, model);
		roModel = roModel;
		...
		add(new ExternalLink("uri", new PropertyModel<String>(model, "uri.toString"),
				new PropertyModel<URI>(model, "uri")));
		add(new EditableTextPanel("titlePanel", new AnnotationTripleModel(model,
				URI.create(DCTerms.title.getURI()), true, false), false).setCanDelete(false));
		...
		add(new EditableTextPanel("descriptionPanel", new AnnotationTripleModel(model,
				URI.create(DCTerms.description.getURI()), true, false), true).setCanDelete(false));
	}
	...
	protected void onConfigure() {
		...
		nestedRO.setVisible(getDefaultModelObject() != null
				&& ((Resource) getDefaultModelObject()).isNestedRO());
	};
