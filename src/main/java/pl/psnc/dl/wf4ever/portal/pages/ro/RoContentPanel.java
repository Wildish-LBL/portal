...
/**
 * A panel for exploring the RO. On the left side there is a list of
 * folders/files. The user selects one folder at a time, if there are any. The
 * folder is the selected resource unless the user selects a resource inside
 * this folder.
 * 
...
public class RoContentPanel extends Panel {
	...
	static {
		try (InputStream mimeTypesIs = CreateROThread.class.getClassLoader().getResourceAsStream(
				"mime.types")) {
	...
	public RoContentPanel(String id, final IModel<ResearchObject> model) {
		...
		IModel<List<Resource>> unrootedResourcesModel = new PropertyModel<List<Resource>>(
				getDefaultModel(), "resourcesWithoutFolders");
		IModel<List<Folder>> rootFolders = new PropertyModel<List<Folder>>(getDefaultModel(),
				"rootFolders");
		IModel<List<Folder>> allFolders = new PropertyModel<List<Folder>>(getDefaultModel(),
				"allFolders");
		...
		add(new FolderContentsPanel("folder-contents", folderModel, resourceModel, rootFolders,
				unrootedResourcesModel));
		add(new ResourceActionsPanel("resource-actions", resourceModel));
		add(new ResourceSummaryPanel("resource-summary", resourceModel, folderModel, model));
		add(new CommentsList("resource-comments", resourceModel));
		add(new AdvancedAnnotationsPanel("advanced-annotations", "resource-basic-view",
				resourceModel));
	...
	private void onResourceMoveClicked(ResourceMoveClickedEvent event) {
		IModel<List<Folder>> allFolders = new PropertyModel<List<Folder>>(getDefaultModel(),
				"allFolders");
		MoveResourceModal moveResourceModal2 = new MoveResourceModal("move-resource-modal",
				allFolders);
	 ...
	 * @throws ROSRSException
	 *             deleting and adding the folder entry caused an unexpected
	 *             response
	 * @throws ROException
	 ...
	 */
	private void onResourceAdd(ResourceAddReadyEvent event) throws ROSRSException, IOException,
			ROException {
		...
		if (event.getUploadedFile() != null) {
			...
			resource = researchObject.aggregate(event.getUploadedFile().getClientFileName(), event
					.getUploadedFile().getInputStream(), contentType, sizeRes);
		} else {
		...
		if (event.getResourceTypes() != null && !event.getResourceTypes().isEmpty()) {
			ResourceTypeModel resourceTypeModel = new ResourceTypeModel(new Model<Resource>(
					resource));
			resourceTypeModel.setObject(event.getResourceTypes());
			if (event.getResourceTypes().contains(ResourceType.SKETCH)) // LOG.debug("es sketch  "+this.getParent()
																		// +
																		// " and "+this.getParent().get("ro-summary")
																		// +
																		// " and page "+getPage()
																		// +" and target: "+event.getTarget());
				send(getPage(), Broadcast.BREADTH, new SketchEvent(event.getTarget()));
	 ...
	 *             the manifest is incorrect
	 */
	private void onResourceUpdate(ResourceUpdateReadyEvent event) throws ROSRSException,
			IOException, ROException {
		if (event.getUploadedFile() != null) {
			long sizeRes = -1;
			if (event.getUploadedFile().getSize() > 0)
				sizeRes = event.getUploadedFile().getSize();
			currentResource.update(event.getUploadedFile().getInputStream(), event
					.getUploadedFile().getContentType(), sizeRes);
			boolean isSketch = updateSketch(event);
			if (isSketch)
				send(getPage(), Broadcast.BREADTH, new SketchEvent(event.getTarget()));
		}
	 ...
	 *             the manifest is incorrect
	 */
	private void onFolderAdd(FolderAddReadyEvent event) throws ROSRSException, ROException {
		if (currentFolder != null) {
	...
	/**
	 * Change the current folder, set the current resource to it and post
	 * events.
	 * 
	 * @param newFolder
	 *            the new folder
